...
async function perform() {
    const {IMPORT_SAMPLE_CATEGORIES} = ADMIN_AJAX_URLS;
    return await getApi("POST", IMPORT_SAMPLE_CATEGORIES, null);
}
export function handleImportDefSampleData() {
    showConfirmDialog({
        title: 'Thông báo',
        message: 'Bạn đang thực hiện tạo dữ liệu mô phỏng mặc định cho phần danh mục. Dữ liệu hiện tại sẽ bị ghi đè, bạn có muốn thực hiện thao tác này không ?',
        yes_label: 'Đồng ý',
        no_label: 'Hủy bỏ',
        yes_callback: async () => {
            showLoadingOverlay();
